...
public interface IVirtualResource {
	 ...
	 * 
	 * @param monitor
	 *            a progress monitor, or <code>null</code> if progress reporting and cancellation
	 *            are not desired
	 */
	 ...
	 * 
	 * <pre>
	 * copy(description, (force ? FORCE : IVirtualResource.NONE), monitor);
	 * </pre>
	 * 
	 * </p>
	 * <p>
	...
	/**
	 * Makes a copy of this project using the given project description. The project's descendents
	 * are copied as well. The description specifies the name, location and attributes of the new
	 * project. After successful completion, corresponding new resources will exist at the given
	 * path; their contents and properties will be copies of the originals. The original resources
	 * are not affected.
	 * <p>
	 * When a resource is copied, its persistent properties are copied with it. Session properties
	 * and markers are not copied.
	 ...
	 * <p>
	 * The <code>SHALLOW</code> update flag controls how this method deals with linked resources.
	 * If <code>SHALLOW</code> is not specified, then the underlying contents of any linked
	 * resources in the project will always be copied in the file system. In this case, the
	 * destination of the copy will never contain any linked resources. If <code>SHALLOW</code> is
	 * specified when a project containing linked resources is copied, new linked resources are
	 * created in the destination project that point to the same file system locations. In this
	 * case, no files on disk under linked resources are actually copied. The <code>SHALLOW</code>
	...
	/**
	 * Creates and returns the marker with the specified type on this resource. Marker type ids are
	 * the id of an extension installed in the <code>org.eclipse.core.resources.markers</code>
	 ...
	 *                </ul>
	 * @see IVirtualResourceRuleFactory#markerRule(IVirtualResource)
	 */
	 ...
	 * from its children. If <code>includeSubtypes</code> is <code>false</code>, only markers
	 * whose type exactly matches the given type are deleted.
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event.
	 * </p>
	 ...
	 * @param depth
	 *            how far to recurse (see <code>IVirtualResource.DEPTH_* </code>)
	 * @exception CoreException
	 *                if this method fails. Reasons include:
	 *                <ul>
	 *                <li> This resource does not exist.</li>
	...
	/**
	 * Returns the marker with the specified id on this resource, Returns <code>null</code> if
	 * there is no matching marker.
	 * 
	 * @param id
	 ...
	 * If <code>includeSubtypes</code> is <code>false</code>, only markers whose type exactly
	 * matches the given type are returned. Returns an empty array if there are no matching markers.
	 * 
	 * @param type
	 *            the type of marker to consider, or <code>null</code> to indicate all types
	 * @param includeSubtypes
	 ...
	 * <code>NULL_STAMP</code> if the resource does not exist or is not local or is not
	 * accessible. The return value is represented as the number of milliseconds since the epoch
	 * (00:00:00 GMT, January 1, 1970). The returned value may not be the same as the actual time
	 * stamp on disk if the file has been modified externally since the last local refresh.
	 * <p>
	 * Note that due to varying file system timing granularities, this value is not guaranteed to
	 * change every time the file is modified. For a more reliable indication of whether the file
	 * has changed, use <code>getModificationStamp</code>.
	 * 
	...
	/**
	 * Returns a marker handle with the given id on this resource. This resource is not checked to
	 * see if it has such a marker. The returned marker need not exist. This resource need not
	 * exist.
	 * 
	 * @param id
	...
	/**
	 * Returns a non-negative modification stamp, or <code>NULL_STAMP</code> if the resource does
	 * not exist or is not local or is not accessible.
	 * <p>
	 * A resource's modification stamp gets updated each time a resource is modified. If a
	 * resource's modification stamp is the same, the resource has not changed. Conversely, if a
	 * resource's modification stamp is different, some aspect of it (other than properties) has
	 * been modified at least once (possibly several times). Resource modification stamps are
	 * preserved across project close/re-open, and across workspace shutdown/restart. The magnitude
	 * or sign of the numerical difference between two modification stamps is not significant.
	 * </p>
	 * <p>
	 ...
	 * <li>moving a resource (source changes to <code>NULL_STAMP</code>, destination changes
	 * from <code>NULL_STAMP</code>)</li>
	 ...
	 * @see IVirtualResource#NULL_STAMP
	 ...
	 * attributes could not be obtained. Attributes that are not supported by the underlying file
	 * system will be set to <code>false</code>.
	 * <p>
	 * Sample usage: <br>
	 * <br>
	 * <code>
	 *  IVirtualResource resource; <br> 
	 *  ... <br>
	 *  IVirtualResourceAttributes attributes = resource.getResourceAttributes(); <br>
	 *  if (attributes != null) {
	 ...
	 * </p>
	 * 
	 * @return the extended attributes from the file system, or <code>null</code> if they could
	 *         not be obtained
	 ...
	 * @param key
	 *            the qualified name of the property
	 * @return the value of the session property, or <code>null</code> if this resource has no
	 *         such property
	 * @exception CoreException
	 *                if this method fails. Reasons include:
	 *                <ul>
	...
	/**
	 * Returns whether this resource subtree is marked as derived. Returns <code>false</code> if
	 ...
	 * are always local.
	 * <p>
	 * When a resource is not local, its content and properties are unavailable for both reading and
	 * writing.
	 * </p>
	 * 
	 * @param depth
	 *            valid values are <code>DEPTH_ZERO</code>, <code>DEPTH_ONE</code>, or
	 *            <code>DEPTH_INFINITE</code>
	...
	/**
	 * Returns <code>true</code> if this resource has been linked to a location other than the
	 * default location calculated by the platform. This location can be outside the project's
	 * content area or another location within the project. Returns <code>false</code> in all
	 * other cases, including the case where this resource does not exist. The workspace root and
	 * projects are never linked.
	 * <p>
	 * This method returns true only for a resource that has been linked using the
	 * <code>createLink</code> method. This method returns false for children of linked resources.
	 * </p>
	 * 
	 ...
	 * <p>
	 * The workspace uses phantom resources to remember outgoing deletions and incoming additions
	 * relative to an external synchronization partner. Phantoms appear and disappear automatically
	 * as a byproduct of synchronization. Since the workspace root cannot be synchronized in this
	 * way, it is never a phantom. Projects are also never phantoms.
	 * </p>
	 * <p>
	 * The key point is that phantom resources do not exist (in the technical sense of
	 * <code>exists</code>, which returns <code>false</code> for phantoms) are therefore
	 * invisible except through a handful of phantom-enabled API methods (notably
	 * <code>IVirtualContainer.members(boolean)</code>).
	 * </p>
	 * 
	 * @return <code>true</code> if this resource is a phantom resource, and <code>false</code>
	 *         otherwise
	 * @see #exists()
	 * @see IVirtualContainer#members(boolean)
	 * @see IVirtualContainer#findMember(String, boolean)
	 * @see IVirtualContainer#findMember(IPath, boolean)
	 * @see ISynchronizer
	 */
	...
	/**
	 * Returns whether this resource and its descendants to the given depth are considered to be in
	 ...
	 * <li>The timestamp in the filesystem has not changed since the last synchronization.</li>
	 * <li>The resource in the workspace is of the same type as the corresponding file in the
	 * filesystem (they are either both files or both folders).</li>
	 * </ul>
	 * A resource is also considered to be in sync if it is missing from both the workspace and the
	 * filesystem. In all other cases the resource is considered to be out of sync.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param depth
	 *            the depth (one of <code>IVirtualResource.DEPTH_ZERO</code>, <code>DEPTH_ONE</code>,
	 *            or <code>DEPTH_INFINITE</code>)
	 * @return <code>true</code> if this resource and its descendants to the specified depth are
	 ...
	 * 
	 * <pre>
	 * move(description, (keepHistory ? KEEP_HISTORY : IVirtualResource.NONE) | (force ? FORCE : IVirtualResource.NONE), monitor);
	 * </pre>
	 * 
	 * </p>
	 * <p>
	 * This operation changes resources; these changes will be reported in a subsequent resource
	 * change event that will include an indication that the resource has been removed from its
	 * parent and that a corresponding resource has been added to its new parent. Additional
	 * information provided with resource delta shows that these additions and removals are related.
	 * </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 * monitor.
	 ...
	 *            system will be tolerated
	 * @param keepHistory
	 *            a flag indicating whether or not to keep local history for files
	 * @param monitor
	 *            a progress monitor, or <code>null</code> if progress reporting is not desired
	 * @exception CoreException
	 *                if this resource could not be moved. Reasons include:
	...
	/**
	 * Renames or relocates this project so that it is the project specified by the given project
	 * description. The description specifies the name, location and attributes of the new project.
	 * After successful completion, the old project and any direct or indirect members will no
	 * longer exist; but corresponding new resources will now exist at the project.
	 * <p>
	 * When a resource moves, its session and persistent properties move with it. Likewise for all
	 ...
	 * default location. In all other cases the directories and files on disk are left untouched. If
	 * the name in the given description is the same as this project's name and the location is
	 * different, then the project contents will be moved to the new location. All other parts of
	 * the given description are ignored.
	 * </p>
	 * <p>
	 * The <code>FORCE</code> update flag controls how this method deals with cases where the
	 * workspace is not completely in sync with the local file system. If <code>FORCE</code> is
	 ...
	 * If <code>SHALLOW</code> is not specified, then the underlying contents of any linked
	 * resource will always be moved in the file system. In this case, the destination of the move
	 * will not contain any linked resources. If <code>SHALLOW</code> is specified when a project
	 * containing linked resources is moved, new linked resources are created in the destination
	 * project pointing to the same file system locations. In this case, no files on disk under any
	 * linked resource are actually moved. The <code>SHALLOW</code> update flag is ignored when
	 ...
	 * relative to the local file system. Creations, deletions, and changes detected in the local
	 * file system will be reflected in the workspace's resource tree. This resource need not exist
	 * or be local.
	 * <p>
	 * This method may discover changes to resources; any such changes will be reported in a
	 * subsequent resource change event.
	 * </p>
	 * <p>
	 * If a new file or directory is discovered in the local file system at or below the location of
	 * this resource, any parent folders required to contain the new resource in the workspace will
	 ...
	 * @see IVirtualResource#DEPTH_ONE
	 * @see IVirtualResource#DEPTH_INFINITE
	 * @see IVirtualResourceRuleFactory#refreshRule(IVirtualResource)
	 */
	public void refreshLocal(int depth, IProgressMonitor monitor) throws CoreException;

	/**
	 * Reverts this resource's modification stamp to a previous value. This is intended to be used
	 * by a client that is rolling back or undoing a previous change to this resource. It is the
	 * caller's responsibility to ensure that the value of the reverted modification stamp matches
	 * this resource's modification stamp prior to the change that has been rolled back.
	 * <p>
	 * Reverting the modification stamp will <b>not</b> be reported in a subsequent resource change
	 * event.
	 * <p>
	 * Note that a resource's modification stamp is unrelated to the local time stamp for this
	 * resource on disk, if any. A resource's local time stamp is modified using the
	 * <code>setLocalTimeStamp</code> method.
	 * 
	 * @param value
	 *            A non-negative modification stamp value
	 * @exception CoreException
	 *                if this method fails. Reasons include:
	 *                <ul>
	 *                <li> This resource does not exist.</li>
	 ...
	 * <p>
	 * A <b>derived</b> resource is a regular file or folder that is created in the course of
	 * translating, compiling, copying, or otherwise processing other files. Derived resources are
	 * not original data, and can be recreated from other resources. It is commonplace to exclude
	 * derived resources from version and configuration management because they would otherwise
	 * clutter the team repository with version of these ever-changing files as each user
	 * regenerates them.
	 * </p>
	 * <p>
	 * If a resource or any of its ancestors is marked as derived, a team provider should assume
	 * that the resource is not under version and configuration management <it>by default</it>.
	 * That is, the resource should only be stored in a team repository if the user explicitly
	 * indicates that this resource is worth saving.
	 * </p>
	 * <p>
	 * Newly-created resources are not marked as derived; rather, the mark must be set explicitly
	 * using <code>setDerived(true)</code>. Derived marks are maintained in the in-memory
	 * resource tree, and are discarded when the resources are deleted. Derived marks are saved to
	 * disk when a project is closed, or when the workspace is saved.
	 * </p>
	 * <p>
	 * Projects and the workspace root are never considered derived; attempts to mark them as
	 * derived are ignored.
	 * </p>
	 * <p>
	 * This operation does <b>not</b> result in a resource change event, and does not trigger
	 * auto-builds.
	 * </p>
	 * 
	 * @param isDerived
	 *            <code>true</code> if this resource is to be marked as derived, and
	...
	/**
	 * Set whether or not this resource and its members (to the specified depth) are expected to
	 * have their contents (and properties) available locally. The workspace root and projects are
	 * always local and attempting to set either to non-local (i.e., passing <code>false</code>)
	 ...
	 * 
	 * @param flag
	 *            whether this resource should be considered local
	 * @param depth
	 *            valid values are <code>DEPTH_ZERO</code>, <code>DEPTH_ONE</code>, or
	 *            <code>DEPTH_INFINITE</code>
	 * @param monitor
	...
	/**
	 * Sets the local time stamp on disk for this resource. The time must be represented as the
	 * number of milliseconds since the epoch (00:00:00 GMT, January 1, 1970). Returns the actual
	 * time stamp that was recorded. Due to varying file system timing granularities, the provided
	 * value may be rounded or otherwise truncated, so the actual recorded time stamp that is
	 * returned may not be the same as the supplied value.
	 * 
	 * @param value
	 *            a time stamp in millseconds.
	...
	/**
	 * Sets the value of the persistent property of this resource identified by the given key. If
	 * the supplied value is <code>null</code>, the persistent property is removed from this
	 * resource. The change is made immediately on disk.
	 * <p>
	 * Persistent properties are intended to be used by plug-ins to store resource-specific
	 * information that should be persisted across platform sessions. The value of a persistent
	 * property is a string which should be short (i.e., under 2KB). Unlike session properties,
	 * persistent properties are stored on disk and maintained across workspace shutdown and
	 * restart.
	 * </p>
	 * <p>
	 ...
	 *            the qualified name of the property
	 * @param value
	 *            the string value of the property, or <code>null</code> if the property is to be
	 *            removed
	 * @exception CoreException
	 *                if this method fails. Reasons include:
	 *                <ul>
	...
	/**
	 * Sets this resource with the given extended attributues. This sets the attributes in the file
	 * system. Only attributes that are suported by the underlying file system will be set.
	 * <p>
	 * Sample usage: <br>
	 * <br>
	 * <code>
	 ...
	 *            the qualified name of the property
	 * @param value
	 *            the value of the session property, or <code>null</code> if the property is to be
	 *            removed
	 * @exception CoreException
	 *                if this method fails. Reasons include:
	 *                <ul>
	...
	/**
	 * Marks this resource as having changed even though its content may not have changed. This
	 * method can be used to trigger the rebuilding of resources/structures derived from this
	 * resource. Touching the workspace root has no effect.
	 * <p>
	 * This method changes resources; these changes will be reported in a subsequent resource change
	 * event. If the resource is a project, the change event will indicate a description change.
	 * </p>
	 * <p>
	 * This method is long-running; progress and cancellation are provided by the given progress
	 * monitor.
	 ...
	 * @see IVirtualResourceDelta#CONTENT
	 * @see IVirtualResourceDelta#DESCRIPTION
	 */
