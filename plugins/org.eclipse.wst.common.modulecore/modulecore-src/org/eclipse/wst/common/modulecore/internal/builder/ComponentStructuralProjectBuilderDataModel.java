...
public class ComponentStructuralProjectBuilderDataModel extends WTPOperationDataModel{
	...
	private List populateFullModuleBuilderDataModelList() {
		...
		for (int i = 0; i < sortedList.size(); i++) {
			...
			if (id == null)
				break;
			dataModel = ComponentStructuralBuilderExtensionRegistry.getComponentStructuralBuilderDMForServerTargetID(runtimeID, wbModule.getComponentType().getModuleTypeId());
